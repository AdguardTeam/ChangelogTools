...
`@adguard/changelog-tools` is deployed as an **npm package** and a
...
repository `AdguardTeam/ChangelogTools`. GitHub Releases are created on
the public mirror.
...
- **Package name**: `@adguard/changelog-tools`
...
- **Repository**: `AdguardTeam/ChangelogTools` (public mirror)
- **Format**: release created automatically on the public mirror
- **Artifact**: `changelog-tools.tgz` is attached to each release
...
(`AdguardTeam/ChangelogTools`) on every push to `master`, using the
shared `mirror.yml` workflow from `AdGuardSoftwareLimited/actions`.
...
| Trigger | Purpose |
|---|---|
| `workflow_dispatch` | Manual trigger with tag input (e.g., `v0.1.0`) |
...
| `build` | `team-extensions` | Inject version, lint and test via Docker, build `changelog-tools.tgz` |
...
| # | Who | Action |
|---|-----|--------|
| 1 | Developer | Go to **Actions → Prepare release → Run workflow**, enter tag (e.g., `v0.1.0`) |
| 2 | CI | Opens PR: `release-bump/v0.1.0` → `master` with finalized `CHANGELOG.md` |
| 3 | Reviewer | Review the PR body (shows changelog section), approve, merge |
| 4 | CI | `publish-release.yml` auto-fires: creates tag `v0.1.0` → inject → lint → test → build → npm publish → mirror + GitHub Release → Slack |
...
| **Product name** | `@adguard/changelog-tools` |
...
enabled for `AdGuardSoftwareLimited/ext-changelog-tools`.
...
`v0.1.0` or a commit SHA). This manually triggers the release pipeline.
